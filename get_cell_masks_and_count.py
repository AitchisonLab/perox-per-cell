...
print("Using yeast_segmentation to segmenting cells...")

path = sys.argv[1]  # Original imaging file
inputdir = sys.argv[2]  # directory for z-projection of cells channel
maskdir = sys.argv[3]  # Directory for z-projection masks
POdotcut = sys.argv[4]  # Peroxisome segmentation sensitivity parameter
POminarea = sys.argv[5]  # Minimum area for peroxisomes
maxintensity = sys.argv[6]  # Theoretical maximum intensity level of input images
...
rawzprojperox = path + '_Zprojections/perox/' + fileid + '_peroxisome_Zprojection.tiff'
...
page = frames.pages[0]
idesc = page.tags["ImageDescription"].value  # area per pixel followed by a space then the physical units for the area
tokens = idesc.split()
pixarea = tokens[0]
...
for i in cellids:
	...
	for p in peroxidsincell:
		...
		if alreadyassigned:
			...
			maxexistingoverlap = max(POcellArrayOverlap[int(p)])  # Maximum value in row representing overlaps with cells
...
for i in cellids:
	...
	if (i % 10) == 0:  # report progress
		print("Finished ", i, " of ", np.max(cellids), " cells")
	...
	# Get cytosolic peroxisomal signal within the cell using the maximum Z-projection of the original peroxisome image
...
df = pd.DataFrame({'image ID': image_id, 'cell ID': cell_id, 'num peroxisomes in cell': num_per,
				   'cell area': cell_area, 'total peroxisome area in cell': totalPOarea, 'total perox channel signal': totalPOsignal,
...
with pd.ExcelWriter(head + '/' + fileid + '_perox_per_cell.xlsx') as writer:
	...
	df.to_excel(writer, sheet_name='ByCell', columns=["image ID", "cell ID", "num peroxisomes in cell", areacolname, peroxareacolname,
		"total perox channel signal", "cytosolic perox signal", "signal from peroxisomes assigned to other cells"], index=False)
	set_column_widths(writer.sheets['ByCell'], df, 33)  # set column widths

	df_PO.to_excel(writer, sheet_name='ByPeroxisome', columns=["image ID", "cell ID", "num peroxisomes in cell", areacolname,
...
print("Done computing peroxisomes per cell.")
